...
def rescale_segment( segment, size = [28,28], pad = 0 ):
    ...
    if len(segment.shape) == 3 : # Non Binary Image
        ...
        ret,segment = cv2.threshold(segment,0,255,cv2.THRESH_BINARY)
    m,n = segment.shape
...
def extract_segments(img, pad=40, reshape = 0,size = [28,28], area = 100, threshold = 150,
...
def load_models(path, ver = 1, names =None ):
    ...
    if ver == 1 :
        ...
        ada1,ada2,ada3,rf,mlp,cnn = ['Adaboost_stage1_ver2.1.sav','Adaboost_digits_ver2.1.sav',
                                         ...
                                         'MLP_singlestage_ver2.1', 'CNN_ver2.1']
    elif ver == 0 :
...
def predict(temp, label_class, prob = 1, adastage1 = None,adadigits= None, adachars= None
